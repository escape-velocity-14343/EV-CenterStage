...
public class GVFFollower {
    ...
    public GVFFollower(double correctionScalar, double currVeloScalar, PIDController headingPID, double pathEndGain, double _curvatureGain, double curvatureTval) {
    ...
    /**
     * Use any distance unit you would like, but ensure that they are the same unit. YOUR VELOCITY OUTPUTS WILL COME BACK IN THIS SAME UNIT. All time units should be in millis. 
     * @param maxaccel The true max acceleration of your robot should be (Max change in velocity)/(millis to change). You may use any value for maxaccel as long as it is within [0, true max accel].
     */
    ...
    public void initCurvature(int acc) {
     ...
     */
    public Vector getMovementVector(Pose2D pose) {
...
//        }
        ...
        mainvect.scale(Math.exp(-correctionFactor));
    ...
    public void updateTrajectory() {
        ...
        if (Vector.dist(lastPose.getPosition(), trajectory.getValue(trajectory.trajectorySegments.get(currindex).length, currindex)) <= positionTolerance &&
            Angle.normalize(Math.abs(headingTrajectory.getValue(trajectory.trajectorySegments.get(currindex).poseSpline.constrain(tval)).get(0)-lastPose.getAngle())) <= angleTolerance) {
     ...
     */
    public void update(Pose2D pose, double millisElapsed) {
        velocity = Vector.add(pose.getPosition(), lastPose.getPosition().scale(-1)).scale(1/millisElapsed);
        lastPose = pose;
        updateTrajectory();
        if (!isTimeout) {
            movementvect = getMovementVector(pose);
            rotationCorrection = rotPID.get(Angle.normalize(headingTrajectory.getValue(trajectory.trajectorySegments.get(currindex).poseSpline.constrain(tval)).get(0)-pose.getAngle()), millisElapsed);
        }
    }
